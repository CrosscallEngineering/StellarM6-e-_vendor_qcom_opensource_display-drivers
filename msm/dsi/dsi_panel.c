...
extern int ff_suspend_resume(bool is_suspend);
...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	/*vddio en gpio*/
	if (gpio_is_valid(panel->vddio_en_gpio)) {
		rc = gpio_request(panel->vddio_en_gpio, "vddio_en_gpio");
		if (rc) {
			DSI_WARN("request for vddio_en_gpio failed, rc=%d\n",
				 rc);
			panel->vddio_en_gpio = -1;
			rc = 0;
		}
		rc = gpio_direction_output(panel->vddio_en_gpio, 1);
		if (rc < 0) {
			pr_err("failed to set output");
			gpio_free(panel->vddio_en_gpio);
			return rc;
		}
	}
	...
	/*dsi2dp muxsel gpio*/
	if (gpio_is_valid(panel->dsi2dp_muxsel_gpio)) {
		rc = gpio_request(panel->dsi2dp_muxsel_gpio, "dsi2dp_muxsel_gpio");
		if (rc) {
			DSI_WARN("request for dsi2dp_muxsel_gpio failed, rc=%d\n",
				 rc);
			panel->dsi2dp_muxsel_gpio = -1;
			rc = 0;
		}
		rc = gpio_direction_output(panel->dsi2dp_muxsel_gpio, 0);//1:dp 0:lcd
		if (rc < 0) {
			pr_err("failed to set output");
			gpio_free(panel->dsi2dp_muxsel_gpio);
			return rc;
		}
	}
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (!ts_supend_need_power_reset_high()) {
		...
		if(gpio_is_valid(panel->vddio_en_gpio)){
			rc = gpio_direction_output(panel->vddio_en_gpio, 1);
			if(rc){
				DSI_ERR("[%s] failed to enable vddio, rc=%d\n", panel->vddio_en_gpio, rc);
				goto exit;
			}
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (!ts_supend_need_power_reset_high()){
		...
		if(gpio_is_valid(panel->vsn_en_gpio)){
			...
			if(rc){
				DSI_ERR("[%s] failed to disable vsn, rc=%d\n", panel->vsn_en_gpio, rc);
				return rc;
			}
			usleep_range(5*1000, 5*1000 + 100);
		}
		if(gpio_is_valid(panel->vsp_en_gpio)){
			...
			if(rc){
				DSI_ERR("[%s] failed to disable vsp, rc=%d\n", panel->vsp_en_gpio, rc);
				return rc;
			}
			usleep_range(5*1000, 5*1000 + 100);
		}
		if (gpio_is_valid(panel->vddio_en_gpio)){
			rc = gpio_direction_output(panel->vddio_en_gpio, 0);
			if(rc){
				DSI_ERR("[%s] failed to disable vddio, rc=%d\n", panel->vddio_en_gpio, rc);
				return rc;
			}
			usleep_range(5*1000, 5*1000 + 100);
		}
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->vddio_en_gpio = utils->get_named_gpio(utils->data,
					"qcom,platform-vddio-en-gpio",
					0);
	if (!gpio_is_valid(panel->vddio_en_gpio))
		printk(KERN_ERR "%s:%d vddio en gpio not specified\n", __func__,
			 __LINE__);

	panel->dsi2dp_muxsel_gpio = utils->get_named_gpio(utils->data,
					"qcom,platform-muxsel-gpio",
					0);
	if (!gpio_is_valid(panel->dsi2dp_muxsel_gpio))
		printk(KERN_ERR "%s:%d dsi2dp_muxsel_gpio gpio not specified\n", __func__,
			 __LINE__);
error:
	return rc;
}
