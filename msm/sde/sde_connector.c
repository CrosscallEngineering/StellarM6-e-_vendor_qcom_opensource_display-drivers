...
int sunlight_flag = 0;
int last_brightness = 255;
int coefficient = 800;
extern int sunlight_led_init(void);
EXPORT_SYMBOL(sunlight_flag);
EXPORT_SYMBOL(s_backlight);
...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if(0 != brightness){
		...
		if((sunlight_flag == 0)||(sunlight_flag == 6)||(sunlight_flag == 7)){
			brightness = coefficient * brightness;
			brightness /= 1000;
			if(brightness < 1){
				brightness = 1;
			}
		}else{
			brightness = coefficient * last_brightness;
			brightness /= 1000;	
		}
	}
	SDE_INFO("sde set backlight sunlight %d coefficient = %d brightness = %d last_brightness = %d\n", sunlight_flag, coefficient, brightness, last_brightness);
#endif/*CONFIG_BOOST_BACKLIGHT_ENABLE*/
	...
	if(sunlight_flag == 6){
		bl_lvl = 0;
		printk("DP connected... flag:%d bl_lvl=%d\n", sunlight_flag, bl_lvl);
	}
#endif/*CONFIG_BOOST_BACKLIGHT_ENABLE*/
...
int sunlight_set_sde_backlight(struct backlight_device *bd){
	int rc = 0,i;
	printk("sunlight_set_sde_backlight coefficient=%d\n",coefficient);
	if((sunlight_flag == 6)||(sunlight_flag == 7)){
...
EXPORT_SYMBOL(sunlight_set_sde_backlight);
#endif/*CONFIG_BOOST_BACKLIGHT_ENABLE*/
					...
					struct drm_device *dev)
{
	...
	#ifdef CONFIG_LCD_DEFAULT_BRIGHTNESS
	props.brightness = CONFIG_LCD_DEFAULT_BRIGHTNESS;
	#else
